...
class UserRegistrationProvider {
  Future<List<UserRegistrationModel>> postUserRegistration(
    ...
    List<UserRegistrationModel> jsonRegistrationModel = [];
    if (response.statusCode == 201) {
      try {
        final jsonRegistrationList = jsonDecode(response.body) as List<dynamic>;
        final jsonRegistrationModel = jsonRegistrationList
            .map((dynamic json) => UserRegistrationModel.fromJson(json))
            .toList();
